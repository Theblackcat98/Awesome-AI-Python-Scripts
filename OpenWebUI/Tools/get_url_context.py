...
class Tools:
    class Valves(BaseModel):
        ...
        GLOBAL_JINA_API_KEY: str = Field(
            default="",
            description="(Optional) Jina API key for r.jina.ai. Allows a higher rate limit when scraping. Used when a User-specific API key is not available.",
        )
    ...
    class UserValves(BaseModel):
        ...
        JINA_API_KEY: str = Field(
            default="",
            description="(Optional) Jina API key for r.jina.ai. Allows a higher rate limit when scraping.",
        )
    ...
    async def web_scrape(
        ...
        """
        Scrape and process a web page using the r.jina.ai service.

        :param url: The URL of the web page to scrape. For authenticated sites, include tokens as query parameters in the URL itself.
        ...
        headers = {
            ...
            "X-Return-Format": "markdown",  # Ensures clean markdown output
        }

        # Use Jina API Key if provided by user or globally
...
class WebScrapeTest(unittest.IsolatedAsyncioTestCase):
    async def test_web_scrape(self):
        ...
        self.assertIn("Scraping Sandbox", content)
