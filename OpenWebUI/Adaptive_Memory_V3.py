...
Your output must be valid JSON only. No additional text.""",
    ...
    async def _find_memory_clusters(
        ...
        logger.debug(
            f"Summarization: Found {len(eligible_memories)} memories older than {min_age_days} days."
        )
        ...
        if strategy in ["embeddings", "hybrid"] and self.embedding_model:
            logger.debug(
                f"Clustering eligible memories using embeddings (threshold: {threshold})..."
            )
            ...
            for mem in eligible_memories:
                ...
                if mem_id not in self.memory_embeddings:
                    try:
                        ...
                        logger.warning(
                            f"Failed to generate embedding for memory {mem_id} during clustering: {e}"
                        )
                ...
                for other_mem in temp_eligible:
                    ...
                    try:
                        ...
                        if similarity >= threshold:
                            ...
                            remaining_after_pop.append(
                                other_mem
                            )  # Keep for next iteration
                    except Exception as e:
                        logger.warning(
                            f"Error comparing embeddings for {current_id} and {other_id}: {e}"
                        )
                        remaining_after_pop.append(other_mem)

                temp_eligible = (
                    remaining_after_pop  # Update list for next outer loop iteration
                )

                if len(cluster) >= self.valves.summarization_min_cluster_size:
                    embedding_clusters.append(cluster)
                    logger.debug(
                        f"Found embedding cluster of size {len(cluster)} starting with ID {current_id}"
                    )
            logger.debug(
                f"Identified {len(embedding_clusters)} potential clusters via embeddings."
            )
        ...
        if strategy in ["tags", "hybrid"]:
            ...
            for candidate in cluster_candidates:
                ...
                current_cluster = [
                    mem for mem in candidate if mem.get("id") not in processed_ids
                ]
                if len(current_cluster) >= self.valves.summarization_min_cluster_size:
                    ...
                    logger.debug(
                        f"Found tag cluster of size {len(current_cluster)} based on tags: {[t for t,mems in tag_map.items() if candidate[0] in mems]}"
                    )
        ...
        if strategy == "hybrid":
            ...
            logger.debug(
                f"Combining {len(embedding_clusters)} embedding clusters and {len(tag_clusters)} tag clusters for hybrid strategy."
            )
        ...
        try:
            ...
            try:
                if self.embedding_model:
                    ...
                    logger.warning(
                        "Embedding model not available for user message encoding."
                    )
                    # If no embedding model, cannot use vector similarity, fallback depends on config
                    if not self.valves.use_llm_for_relevance:
                        logger.warning(
                            "Cannot calculate relevance without embedding model or LLM fallback."
                        )
...
Current datetime: {current_datetime.strftime('%A, %B %d, %Y %H:%M:%S')} ({current_datetime.tzinfo})"""  # Removed escaping backslashes
                    ...
                    if not llm_relevance_results or not isinstance(
                        ...
                        logger.warning(
                            "Failed to parse relevance data from LLM response for uncached items."
                        )
